function Tank(x, y, src, speed , type)
{
    ...
    Tank.instanceIndex++;
    this.myIndex = Tank.instanceIndex;
}
...
Tank.prototype.shot = function()
{
	...
	{	
		...
		var bullet = new Bullet(this.x,this.y,this.type,this.dir,null,(this.power+this.powerAdd)*isCrit,this);
		bullets.push(bullet);
	}
};
...
// 接受伤害
Tank.prototype.sustainDmg = function(dmg)
{
    var realDmg = dmg - this.armor;
    var isHit = parseInt(Math.random()*1000);
    //realDmg = realDmg
    this.life -= realDmg;
    if (this.life < 0) {
        this.life = 0;
...
// 接受治疗
Tank.prototype.gainHeal = function(power)
{
    this.life += power;
}

// 接受获得奖励
Tank.prototype.gainPower = function(power)
{
    this.powerAdd += power;
}
...
function findTankByIndex(myIndex)
{
	for(var i = 0; i < tanks.length ; i ++)
	{
		if (tanks[i].myIndex == myIndex) {
            return tanks[i];
        }
	}
}
